...
{
	if (SYSERR == lock (lck, WRITE, DEFAULT_LOCK_PRIO)) {
		kprintf("bingo! test case passed!\n");
		return;
	}
	kprintf ("  %s: acquired lock, sleep 2s\n", msg);
	sleep (2);
...
void testx ()
{
	...
	pid1 = create(reader1, 2000, 20, "reader a", 2, "reader a", lck);
	pid2 = create(writer1, 2000, 20, "reader b", 2, "reader b", lck);
...
void reader4 (char *msg, int lck)
{
        ...
        kprintf ("  %s: to acquire lock\n", msg);
        lock (48, READ, DEFAULT_LOCK_PRIO);
        kprintf ("  %s: acquired lock\n", msg);
        kprintf ("  %s: to release lock\n", msg);
        releaseall (1, 48);
}
...
void writer4 (char *msg, int lck)
{
        kprintf ("  %s: to acquire lock\n", msg);
        lock (lck, WRITE, DEFAULT_LOCK_PRIO);
        lock (48, WRITE, DEFAULT_LOCK_PRIO);
        kprintf ("  %s: acquired lock, sleep 10s\n", msg);
        sleep (10);
        kprintf ("  %s: to release lock\n", msg);
        releaseall (1, lck, 48);
...
void test4 ()
{
        ...
        rd1 = create(reader3, 2000, 25, "reader3", 2, "reader A", lck);
        rd2 = create(reader3, 2000, 30, "reader3", 2, "reader B", lck);
        rd3 = create(reader4, 2000, 27, "reader3", 2, "reader C", lck);
        wr1 = create(writer4, 2000, 20, "writer3", 2, "writer", lck);
	...
	kill (rd2);
	sleep (1);
	assert (getprio(wr1) == 27, "Test 3 failed");
